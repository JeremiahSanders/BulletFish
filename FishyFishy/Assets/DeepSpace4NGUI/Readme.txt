DeepSpace for NGUI
v.1.5 March 2014
Art and code: Ewa Aguero Padilla
Contact: avee@muhagames.com


DeepSpace for NGUI is a skin that requires NGUI v.3.5.3 or above to work (though future NGUI releases might have some compatibility issues - if you encounter any please contact me so I can send you a fix). It contains a scene (DeepSpaceNGUI_TestScene) with NGUI scripts, which allow you to see all the elements on a simple working example. 
There are two fonts used in the skin, both are free for both commercial and private use - see .txt files for each font. 
If you are still using earlier version of Unity or NGUI, e-mail me and I'll get you a previous release.





Changelog:

1.5 March 2014
- Fixed a glitch with scrollbars' thumb sizes

1.4 February 2014
- Fixed compatibility issues with NGUI 3.4.9/Unity 4.3.4
- Removed NGUI 2.7 compatible package that was added with DeepSpace4NGUI v 1.3 (though feel free to e-mail me if you still need it)

1.3 October 2013
- Now includes a package fully compatible with NGUI 3.0.2. (also requires Unity 4.2)

1.2 August 2013

- Set DeepSpaceNGUI_Atlas.png texture type to Advanced which should fix texture problems in Unity 4.2

1.1 July 2013

- Fixed depth issues on a few widgets

- Cleaned-up scripts on Radio Buttons and H. Scrollbar



1.0 June 2013

- Initial version.